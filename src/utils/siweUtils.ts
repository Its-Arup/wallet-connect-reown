...
// call the server to get a nonce
 const getNonce = async () : Promise<string> => {
    const res = await fetch(BASE_URL + "/api/user/nonce", {
        method: "GET",
        credentials: 'include',
      });
    if (!res.ok) {
...
// call the server to get the session
 const getSession = async () => {
  ...
  const isValidData = typeof data === 'object' && typeof data.address === 'string' && typeof data.chainId === 'number' && (typeof data.user === 'object' || data.user === null);
